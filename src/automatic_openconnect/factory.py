...
This lets main.py do::
    ...
    with auto_vpn_session(config_data), make_sender(...) as sender, ...:
        _run_workflow(...)
...
def make_vpn_session(config_data: dict):
    ...
    if sys.platform == "linux":
        from ._linux import auto_vpn_session
...
# Public name used by the package API and by termino's integration
# (`from automatic_openconnect import auto_vpn_session`). The cross-platform
# factory IS the public entry point; the per-OS sessions are internal.
auto_vpn_session = make_vpn_session
