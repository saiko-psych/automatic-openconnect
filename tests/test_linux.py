# -*- coding: utf-8 -*-
"""
Tests for automatic_openconnect._linux.
...
from automatic_openconnect._linux import (
    VPNError,
    auto_vpn_session,
    is_vpn_up,
)
...
class TestStartTunnelPipeDetach(unittest.TestCase):
    ...
    def test_uses_devnull_and_new_session(self):
        from automatic_openconnect._linux import _start_tunnel

        with mock.patch("automatic_openconnect._linux._resolve_tool", return_value="/usr/sbin/openconnect"), \
             mock.patch("automatic_openconnect._linux.subprocess.run") as run, \
             mock.patch("automatic_openconnect._linux.Path") as PathMock, \
             mock.patch("automatic_openconnect._linux.time.sleep"):
...
class TestCleanupOnSetupFailure(unittest.TestCase):
    ...
    def test_stop_tunnel_called_when_start_tunnel_raises(self):
        cfg = {"auto_vpn": {"enabled": True,
                            "user_email": "x@example.org"}}
        with mock.patch.object(sys, "platform", "linux"), \
             mock.patch("automatic_openconnect._linux.is_vpn_up", return_value=False), \
             mock.patch("automatic_openconnect._linux._check_keyring_credentials"), \
             mock.patch("automatic_openconnect._linux._authenticate",
                        return_value=("host", "cookie", "fp")), \
             mock.patch("automatic_openconnect._linux._start_tunnel",
                        side_effect=VPNError("simulated tunnel failure")), \
             mock.patch("automatic_openconnect._linux._stop_tunnel") as stop:
...
class TestStopTunnelSudoersCompat(unittest.TestCase):
    ...
    def test_kills_each_pid_via_kill_not_pkill_dash_f(self):
        from automatic_openconnect._linux import _stop_tunnel
        with mock.patch.object(sys, "platform", "linux"), \
             mock.patch("automatic_openconnect._linux.subprocess.run") as run, \
             mock.patch("automatic_openconnect._linux.time.sleep"):
            run.side_effect = [
